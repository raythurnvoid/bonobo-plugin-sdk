...
- `plugin.data.user-write` — the page may create, change, and delete documents in that store as the acting member. The UI token never carries a write scope: the write runs through the app's own member-attributed mutations on the page's Convex client (see [Plugin data on the page's own Convex client](#plugin-data-on-the-pages-own-convex-client)). Declaring it also requires `plugin.data.read`.
