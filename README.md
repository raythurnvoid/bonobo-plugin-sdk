...
| page → host | `bonobo:data-user-write`       | `bridgeNonce`, `requestId`, `op` (`"append" \| "put" \| "remove" \| "putOwned" \| "removeOwned"`), `collection`, `keyPrefix?`, `key?`, `value?`, `clientRequestId?` |
...
| host → page | `bonobo:data-update`           | `bridgeNonce`, `subscriptionId`, `docs` (`PublicDoc[] \| null`)                                                                             |
| host → page | `bonobo:data-user-write-result` | `bridgeNonce`, `requestId`, `result` (`{ _yay: { key } } \| { _nay: { message } }`)                                                        |
...
- `client.data.watch({ collection, keyPrefix?, limit }, onUpdate)` opens one reactive subscription and returns an unsubscribe function. Each `bonobo:data-update` delivers the subscription's whole current window (`PublicDoc[]`) — replace it, do not accumulate. The host caps `limit` at 100 and allows at most 8 active watches per frame. `docs: null` means the subscription is dead (authorization lost, cap exceeded, or query error): the SDK delivers the `null` once and drops the registration, so a later unsubscribe is a no-op.
- `client.data.append/put/remove/putOwned/removeOwned` post one `bonobo:data-user-write` each and resolve with the host's `result` as-is (`{ _yay: { key } }` or `{ _nay: { message } }`); they reject only when the host does not answer within 10 seconds. `putOwned`/`removeOwned` write a member-owned document: the host stores it under `<key>:<userId>` (the `userId` from the init context), so the caller's `key` may be at most `128 - userId.length - 1` characters.
